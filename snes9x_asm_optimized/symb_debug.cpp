...
void ExceptionHandler(TExcType exc)
{
	...
	void debugPrintFileInit()
	{
		...
		// try to open
		TInt    res = file->Open(*fs, _L("C:\\data\\AntSnes\\snes9x.log"), EFileWrite|EFileShareAny);
		...
		// try to create
		if(res) res = file->Create(*fs, _L("C:\\data\\AntSnes\\snes9x.log"), EFileWrite|EFileShareAny);
